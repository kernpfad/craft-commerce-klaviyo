...
- **Restore action**: `https://yoursite.test/actions/commerce-klaviyo/cart/restore?number={{ event.OrderId }}` — redirects to the same load-cart URL for incomplete carts; completed or missing carts return 404.
...
List IDs always use Klaviyo’s subscribe (double opt-in aware) endpoint. Optional `forward` redirects after success. Common form fields: `profile[]`, `event[]`, `lists[]` / `list`, and optional `event[trackOrder]`.
